<?php include 'header.php' ?>
<!-- index.php -->
<section class="main">
    <div class="main--cover">
        <img src="" alt="">
        <h1>Mun portfolio jee</h1>
    </div>

    <div class="main--introduction">
        <img src="" alt="">
        <h2>About ja sillee</h2>
        <div>Elit rerum adipisci ipsa quaerat facilis! Dolorum cum voluptatem consequatur nemo voluptatibus debitis eligendi. Maxime quibusdam maiores consectetur deserunt autem in? Ad doloremque dolorem ad perspiciatis unde animi. Nostrum dolorem.
            Consectetur quae quod a tempore eius quas? Perferendis voluptatem commodi enim maiores aliquam esse, earum! Nobis pariatur qui nostrum laboriosam harum cumque culpa temporibus tempore cum tempore natus consequuntur. Excepturi.
        </div>
    </div>

    <div class="main--skills">
        <h2>Skillit jne.</h2>
        <p>Sit non fugit nam laborum temporibus dolorem earum totam harum! Quod totam quo voluptatem blanditiis unde adipisci voluptas, exercitationem maiores dicta doloremque labore! Sequi culpa ratione ex in sapiente. Nisi?</p>
    </div>

    <div class="main--works">
        <h2>I made this</h2>
        <div>
            <img src="" alt="">
            <div>
                <a href="">Linkki sivuun</a>
                <p>Lorem quae quisquam quas molestiae velit a quae ducimus sapiente ipsum impedit. Similique ducimus quod voluptate iusto assumenda. Quisquam expedita perferendis necessitatibus a sunt. Veniam soluta iste illo nemo eligendi.</p>
            </div>
        </div>
    </div>
</section>

<?php include 'footer.php' ?>
